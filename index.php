...
function save_blog_thumbnail(?array $file): ?string { if (!$file || empty($file['tmp_name'])) return null; if (($file['error'] ?? UPLOAD_ERR_OK) !== UPLOAD_ERR_OK || (int)($file['size'] ?? 0) > 5 * 1024 * 1024) throw new RuntimeException('Please upload a thumbnail image under 5 MB.'); $allowed = ['image/jpeg'=>'jpg','image/png'=>'png','image/webp'=>'webp']; $mime = (new finfo(FILEINFO_MIME_TYPE))->file($file['tmp_name']); if (!isset($allowed[$mime])) throw new RuntimeException('Thumbnail must be JPG, PNG, or WEBP.'); $dir = __DIR__.'/uploads/blog'; if (!is_dir($dir) && !mkdir($dir, 0775, true) && !is_dir($dir)) throw new RuntimeException('Blog upload folder could not be created.'); $name = 'thumb-'.date('YmdHis').'-'.bin2hex(random_bytes(5)).'.'.$allowed[$mime]; if (!move_uploaded_file($file['tmp_name'], $dir.'/'.$name)) throw new RuntimeException('Thumbnail upload failed.'); return $name; }
function save_blog_video(?array $file): ?string { if (!$file || empty($file['tmp_name'])) return null; if (($file['error'] ?? UPLOAD_ERR_OK) !== UPLOAD_ERR_OK || (int)($file['size'] ?? 0) > 80 * 1024 * 1024) throw new RuntimeException('Please upload a video under 80 MB, or use the link field for a hosted video instead.'); $allowed = ['video/mp4'=>'mp4','video/webm'=>'webm','video/quicktime'=>'mov']; $mime = (new finfo(FILEINFO_MIME_TYPE))->file($file['tmp_name']); if (!isset($allowed[$mime])) throw new RuntimeException('Video must be MP4, WEBM, or MOV.'); $dir = __DIR__.'/uploads/blog'; if (!is_dir($dir) && !mkdir($dir, 0775, true) && !is_dir($dir)) throw new RuntimeException('Blog upload folder could not be created.'); $name = 'video-'.date('YmdHis').'-'.bin2hex(random_bytes(5)).'.'.$allowed[$mime]; if (!move_uploaded_file($file['tmp_name'], $dir.'/'.$name)) throw new RuntimeException('Video upload failed.'); return $name; }
...
function membership_card_html(array $user): string {
    ...
    return '<div class="membership-card" style="background:'.$badge['gradient'].'"><div class="membership-card-top"><span class="badge-chip '.e($badge['class']).'">'.$badge['icon'].' '.e($badge['label']).'</span><span class="membership-status-pill">● Active Member</span></div><div class="membership-card-body">'.$photoHtml.'<div><h3>'.e($user['name']).'</h3><p>Royal Family TZ Member</p><p class="membership-card-id">Membership ID: '.e($user['membership_id'] ?: 'Pending confirmation').'</p></div></div></div>';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'admin_user_create' && $user && $user['role'] === 'admin') {
        try {
            ...
            $manualId = trim($_POST['membership_id'] ?? '');
            ...
            if ($manualId !== '') { $clash = db()->prepare('SELECT id FROM users WHERE membership_id = ? LIMIT 1'); $clash->execute([$manualId]); if ($clash->fetch()) throw new RuntimeException('That membership ID is already assigned to another member.'); }
            ...
            // Membership ID is derived from the new row's own auto-increment id, so it can never collide with another member's — unless the admin typed one manually.
            $newId = (int)db()->lastInsertId();
            if ($manualId !== '') { db()->prepare('UPDATE users SET membership_id = ? WHERE id = ?')->execute([$manualId, $newId]); }
            elseif ($active) { $prefix = $badge === 'gold' ? 'GOLD' : ($badge === 'silver' ? 'SILVER' : 'MEMBER'); db()->prepare('UPDATE users SET membership_id = ? WHERE id = ?')->execute([$prefix . '-' . str_pad((string)$newId, 6, '0', STR_PAD_LEFT), $newId]); }
    ...
    if ($action === 'admin_user_update' && $user && $user['role'] === 'admin') {
        try {
            ...
            // can never collide — and COALESCE below means an auto-generated ID is never overwritten
            // unless the admin explicitly types a manual ID in the field.
            $manualId = trim($_POST['membership_id'] ?? '');
            if ($manualId !== '') {
                $clash = db()->prepare('SELECT id FROM users WHERE membership_id = ? AND id <> ? LIMIT 1');
                $clash->execute([$manualId, $id]);
                if ($clash->fetch()) throw new RuntimeException('That membership ID is already assigned to another member. Please choose a different one.');
            }
            $autoId = null;
            if ($active) { $prefix = $badge === 'gold' ? 'GOLD' : ($badge === 'silver' ? 'SILVER' : 'MEMBER'); $autoId = $prefix . '-' . str_pad((string)$id, 6, '0', STR_PAD_LEFT); }
            $finalMembershipId = $manualId !== '' ? $manualId : null;
            if (!empty($_POST['password'])) {
                if (strlen($_POST['password']) < 6) throw new RuntimeException('Password must be at least 6 characters.');
                db()->prepare('UPDATE users SET name=?, email=?, role=?, membership_active=?, membership_badge=?, membership_id=COALESCE(?, membership_id, ?), password_hash=? WHERE id=?')->execute([$name, $email, $role, $active, $badge, $finalMembershipId, $autoId, password_hash($_POST['password'], PASSWORD_DEFAULT), $id]);
            } else db()->prepare('UPDATE users SET name=?, email=?, role=?, membership_active=?, membership_badge=?, membership_id=COALESCE(?, membership_id, ?) WHERE id=?')->execute([$name, $email, $role, $active, $badge, $finalMembershipId, $autoId, $id]);
    ...
    if ($action === 'blog_create' && $user && $user['role'] === 'admin') {
        try {
            ensure_blog_table();
            $title = trim($_POST['title'] ?? ''); $description = trim($_POST['description'] ?? ''); $externalLink = trim($_POST['external_link'] ?? '');
            if ($title === '') throw new RuntimeException('Please give the post a title.');
            if ($externalLink !== '' && !filter_var($externalLink, FILTER_VALIDATE_URL)) throw new RuntimeException('The link must be a valid URL, starting with https://');
            $thumbnail = save_blog_thumbnail($_FILES['thumbnail'] ?? null);
            $video = save_blog_video($_FILES['video'] ?? null);
            db()->prepare('INSERT INTO blog_posts (title, description, thumbnail, video, external_link) VALUES (?, ?, ?, ?, ?)')->execute([$title, $description ?: null, $thumbnail, $video, $externalLink ?: null]);
            $_SESSION['flash'] = 'Post published to the blog.';
        } catch (Throwable $e) { $_SESSION['flash'] = 'Post could not be published: ' . $e->getMessage(); }
        header('Location: /admin/blog'); exit;
    }
    if ($action === 'blog_update' && $user && $user['role'] === 'admin') {
        try {
            ensure_blog_table();
            $id = (int)($_POST['post_id'] ?? 0); $title = trim($_POST['title'] ?? ''); $description = trim($_POST['description'] ?? ''); $externalLink = trim($_POST['external_link'] ?? '');
            if (!$id || $title === '') throw new RuntimeException('Please give the post a title.');
            if ($externalLink !== '' && !filter_var($externalLink, FILTER_VALIDATE_URL)) throw new RuntimeException('The link must be a valid URL, starting with https://');
            $thumbnail = save_blog_thumbnail($_FILES['thumbnail'] ?? null);
            $video = save_blog_video($_FILES['video'] ?? null);
            if ($thumbnail && $video) db()->prepare('UPDATE blog_posts SET title=?, description=?, thumbnail=?, video=?, external_link=? WHERE id=?')->execute([$title, $description ?: null, $thumbnail, $video, $externalLink ?: null, $id]);
            elseif ($thumbnail) db()->prepare('UPDATE blog_posts SET title=?, description=?, thumbnail=?, external_link=? WHERE id=?')->execute([$title, $description ?: null, $thumbnail, $externalLink ?: null, $id]);
            elseif ($video) db()->prepare('UPDATE blog_posts SET title=?, description=?, video=?, external_link=? WHERE id=?')->execute([$title, $description ?: null, $video, $externalLink ?: null, $id]);
            else db()->prepare('UPDATE blog_posts SET title=?, description=?, external_link=? WHERE id=?')->execute([$title, $description ?: null, $externalLink ?: null, $id]);
            $_SESSION['flash'] = 'Post updated.';
        } catch (Throwable $e) { $_SESSION['flash'] = 'Post could not be updated: ' . $e->getMessage(); }
        header('Location: /admin/blog'); exit;
    }
    if ($action === 'blog_delete' && $user && $user['role'] === 'admin') {
        try { ensure_blog_table(); db()->prepare('DELETE FROM blog_posts WHERE id = ?')->execute([(int)($_POST['post_id'] ?? 0)]); $_SESSION['flash'] = 'Post removed.'; } catch (Throwable $e) { $_SESSION['flash'] = 'Post could not be removed: ' . $e->getMessage(); }
        header('Location: /admin/blog'); exit;
    }
    if ($action === 'admin_generate_missing_ids' && $user && $user['role'] === 'admin') {
        try {
            ensure_membership_badge_column();
            $count = 0;
            $missing = db()->query("SELECT id, membership_badge FROM users WHERE membership_active = 1 AND (membership_id IS NULL OR membership_id = '')")->fetchAll();
            foreach ($missing as $m) {
                $prefix = $m['membership_badge'] === 'gold' ? 'GOLD' : ($m['membership_badge'] === 'silver' ? 'SILVER' : 'MEMBER');
                $newId = $prefix . '-' . str_pad((string)$m['id'], 6, '0', STR_PAD_LEFT);
                db()->prepare("UPDATE users SET membership_id = ? WHERE id = ? AND (membership_id IS NULL OR membership_id = '')")->execute([$newId, $m['id']]);
                $count++;
            }
            $_SESSION['flash'] = $count > 0 ? "Generated membership IDs for {$count} active member(s)." : 'Every active member already has a membership ID.';
        } catch (Throwable $e) { $_SESSION['flash'] = 'Could not generate IDs: ' . $e->getMessage(); }
        header('Location: /admin/users'); exit;
    }
...
function amount_chip_styles(): string { return <<<'CHIPCSS'
<style>
.amount-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:6px 0 14px}
.amount-chip{position:relative;display:block}
.amount-chip input[type="radio"],.amount-chip input[type="checkbox"]{position:absolute;top:0;left:0;width:100%;height:100%;margin:0;opacity:0;cursor:pointer;z-index:2}
.amount-chip label{position:relative;display:flex;align-items:center;justify-content:center;width:100%;box-sizing:border-box;padding:14px 10px;border:2px solid #e8dfcb;border-radius:12px;background:#fff;cursor:pointer;font-weight:700;text-align:center;font-size:.92rem;line-height:1.25;min-height:48px}
.amount-chip input:checked+label{border-color:#c9a54c;background:#fffef0;box-shadow:0 4px 12px rgba(201,165,76,0.13)}
.amount-chip input:focus-visible+label{outline:2px solid #c9a54c;outline-offset:2px}
...
case '/apply-membership':
    $content = amount_chip_styles().'<div class="success-modal" id="membership-terms-modal"><div class="success-card terms-card"><h2>Foundation Membership Terms & Conditions</h2><div class="terms-body">'
...
case '/contact':
  ...
  try {
    ...
    break;
case '/donate':
    $content = amount_chip_styles().'<div class="success-modal" id="donor-terms-modal"><div class="success-card terms-card"><h2>Donors & Sponsors Terms & Conditions</h2><div class="terms-body">'
...
case '/dashboard':
    try {
        ensure_membership_badge_column();
        $dashFreshStmt = db()->prepare('SELECT membership_active, membership_id, membership_badge FROM users WHERE id = ? LIMIT 1');
        $dashFreshStmt->execute([$user['id']]);
        $dashFresh = $dashFreshStmt->fetch();
        if ($dashFresh) { $user['membership'] = (bool)$dashFresh['membership_active']; $user['membership_id'] = $dashFresh['membership_id']; $user['membership_badge'] = $dashFresh['membership_badge']; $_SESSION['user'] = $user; }
    } catch (Throwable $e) {}
...
case '/admin/blog':
    ensure_blog_table();
    $postRows = ''; try {
        foreach (db()->query('SELECT id, title, description, thumbnail, video, external_link, created_at FROM blog_posts ORDER BY created_at DESC') as $p) {
            $thumbSrc = blog_media_src($p['thumbnail'] ?? null);
            $postRows .= '<div class="card" style="margin-top:1rem"><div class="trip-admin-row">'.($thumbSrc ? '<img src="'.e($thumbSrc).'" alt="" style="width:64px;height:64px;border-radius:10px;object-fit:cover;margin-right:12px">' : '').'<span><strong>'.e($p['title']).'</strong><small>'.($p['video'] ? '🎬 Has video · ' : '').($p['external_link'] ? '🔗 Has link · ' : '').e(date('M j, Y', strtotime($p['created_at']))).'</small></span></div><details><summary class="btn">Edit</summary><form class="form" method="post" enctype="multipart/form-data" style="margin-top:1rem"><input type="hidden" name="action" value="blog_update"><input type="hidden" name="post_id" value="'.(int)$p['id'].'"><label>Title<input name="title" value="'.e($p['title']).'" required></label><label>Description<textarea name="description" rows="3">'.e((string)($p['description'] ?? '')).'</textarea></label><label>Replace thumbnail image<input type="file" name="thumbnail" accept="image/jpeg,image/png,image/webp"><small class="muted">JPG, PNG, or WEBP, up to 5 MB. Leave empty to keep the current one.</small></label><label>Replace video file<input type="file" name="video" accept="video/mp4,video/webm,video/quicktime"><small class="muted">MP4, WEBM, or MOV, up to 80 MB. Leave empty to keep the current one.</small></label><label>Link (channel, playlist, or any page)<input type="url" name="external_link" value="'.e((string)($p['external_link'] ?? '')).'" placeholder="https://youtube.com/@yourchannel"></label><button class="btn gold" type="submit">Save changes</button></form><form method="post" style="margin-top:.5rem" onsubmit="return confirm(\'Delete this post? This cannot be undone.\')"><input type="hidden" name="action" value="blog_delete"><input type="hidden" name="post_id" value="'.(int)$p['id'].'"><button class="btn danger" type="submit">Delete post</button></form></details></div>';
        }
    } catch (Throwable $e) { $postRows = '<p class="muted">Blog table unavailable.</p>'; }
    $content = '<div class="page"><p class="eyebrow">Admin workspace</p><h1>Blog & videos</h1><p class="lead">Publish stories with a thumbnail, an optional video, and a link that sends readers to a channel or any page you choose.</p><details class="admin-action"><summary class="btn gold">Add new post</summary><div class="card"><h3>Create a post</h3><form class="form" method="post" enctype="multipart/form-data"><input type="hidden" name="action" value="blog_create"><label>Title<input name="title" placeholder="Community outreach in Dodoma" required></label><label>Description<textarea name="description" rows="3" placeholder="Short summary shown on the blog page"></textarea></label><label>Thumbnail image<input type="file" name="thumbnail" accept="image/jpeg,image/png,image/webp"><small class="muted">JPG, PNG, or WEBP, up to 5 MB.</small></label><label>Video file (optional)<input type="file" name="video" accept="video/mp4,video/webm,video/quicktime"><small class="muted">MP4, WEBM, or MOV, up to 80 MB. Skip this if you\'d rather just link to a hosted video below.</small></label><label>Link (channel, playlist, or any page)<input type="url" name="external_link" placeholder="https://youtube.com/@yourchannel"></label><button class="btn" type="submit">Publish post</button></form></div></details>'.($postRows ?: '<p class="muted">No posts yet.</p>').'</div>';
    break;
case '/admin/images': $content='<div class="page"><p class="eyebrow">Admin workspace</p><h1>Images</h1><div class="card"><p class="lead">This area is ready for content management. The member and payment records are connected to MySQL.</p><a class="btn" href="/admin">Back to dashboard</a></div></div>'; break;
