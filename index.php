    ...
    try {
        ...
        if ($reference !== '') {
            ...
            if ($status) {
                ...
                if ($status === 'failed') {
                    try {
                        db()->prepare("UPDATE trip_bookings b JOIN transactions t ON t.id = b.transaction_id SET b.status = 'cancelled' WHERE t.order_reference = ?")->execute([$reference]);
                    } catch (Throwable $ignore) {}
                }
            }
        }
...
if ($path === '/api/transaction-status' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        ...
        if ($tx['status'] === 'paid' || $tx['status'] === 'failed') {
            if (($_SESSION['active_payment']['reference'] ?? '') === $reference) {
                unset($_SESSION['active_payment']);
            }
        }
...
if (in_array($path, ['/admin/report-users.csv', '/admin/report-transactions.csv'], true) && $user && $user['role'] === 'admin') {
    ...
    header('Content-Disposition: attachment; filename=' . ($path === '/admin/report-users.csv' ? 'royalfamilytz-users.csv' : 'royalfamilytz-transactions.csv'));
    $out = fopen('php://output', 'w');
    if ($path === '/admin/report-users.csv') {
        fputcsv($out, ['User ID','Name','Email','Role','Membership Tier','Membership ID','Membership Active','Created At']);
        foreach (db()->query('SELECT id, name, email, role, membership_tier, membership_id, membership_active, created_at FROM users ORDER BY created_at DESC') as $row) {
            fputcsv($out, [$row['id'], $row['name'], $row['email'], $row['role'], $row['membership_tier'] ?? 'None', $row['membership_id'] ?? '', $row['membership_active'] ? 'Yes' : 'No', $row['created_at']]);
        }
    } else {
        fputcsv($out, ['Transaction ID','Order Reference','User','Email','Type','Package/Tier','Amount','Currency','Method','Status','Created At']);
        foreach (db()->query('SELECT t.id, t.order_reference, COALESCE(u.name, \'Guest\') AS user_name, COALESCE(u.email, \'\') AS email, t.type, t.tier, t.amount, t.currency, t.method, t.status, t.created_at FROM transactions t LEFT JOIN users u ON u.id = t.user_id ORDER BY t.created_at DESC') as $row) {
            fputcsv($out, [$row['id'], $row['order_reference'], $row['user_name'], $row['email'], $row['type'], $row['tier'], $row['amount'], $row['currency'], $row['method'], $row['status'], $row['created_at']]);
        }
    }
    fclose($out);
    exit;
...
if ($path === '/auth/google/start') {
    if (!google_enabled()) {
        $_SESSION['flash'] = 'Google sign-in is not configured.';
...
if ($path === '/auth/google/callback') {
    try {
        ...
        if ($expectedState === '' || $returnedState === '' || !hash_equals($expectedState, $returnedState)) {
            throw new RuntimeException('Google sign-in session expired.');
        }
        unset($_SESSION['google_oauth_state']);
        $profile = google_exchange_code((string)$_GET['code']);
        ensure_profile_columns();
        $stmt = db()->prepare('SELECT id, name, email, membership_active, membership_tier, role, membership_id, profile_image FROM users WHERE google_id = ? OR email = ? LIMIT 1');
        $stmt->execute([$profile['sub'], strtolower($profile['email'])]);
        $account = $stmt->fetch();
        if ($account) {
            db()->prepare('UPDATE users SET google_id = ?, avatar_url = ?, name = ?, email_verified_at = COALESCE(email_verified_at, NOW()) WHERE id = ?')->execute([$profile['sub'], $profile['picture'] ?? null, $profile['name'] ?? $account['name'], $account['id']]);
        } else {
            ...
            $account = ['id'=>db()->lastInsertId(),'name'=>$profile['name'] ?? $profile['email'],'email'=>strtolower($profile['email']),'membership_active'=>0,'membership_tier'=>null,'role'=>'member','membership_id'=>null,'profile_image'=>null];
        }
        $_SESSION['user'] = [
            'id'=>(int)$account['id'],
            'name'=>$account['name'],
            'email'=>$account['email'],
            'membership'=>(bool)$account['membership_active'],
            'tier'=>$account['membership_tier'] ?? 'Standard',
            'role'=>$account['role'],
            'membership_id'=>$account['membership_id'],
            'profile_image'=>$account['profile_image'] ?? null,
            'avatar_url'=>$profile['picture'] ?? null
        ];
        header('Location: ' . app_base_path() . ($account['role'] === 'admin' ? '/admin' : '/dashboard'));
        exit;
    } catch (Throwable $e) {
        $_SESSION['flash'] = $e->getMessage();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'trip_book' && $user) {
        try {
            ...
            $package = $stmt->fetch();
            if (!$package) {
                throw new RuntimeException('That trip package is no longer available.');
            }
            ...
            if (!empty($result['checkoutLink'])) {
                header('Location: ' . $result['checkoutLink']);
                exit;
            }
            $_SESSION['flash'] = 'Booking created. Complete payment to receive your ticket via email.';
        } catch (Throwable $e) {
            $_SESSION['flash'] = 'Trip booking failed: ' . $e->getMessage();
        }
    ...
    if ($action === 'login') {
        try {
            ...
            if ($account && password_verify($_POST['password'] ?? '', (string)$account['password_hash'])) {
                if ($account['role'] !== 'admin' && empty($account['email_verified_at'])) {
                    $_SESSION['flash'] = 'Please verify your email address before logging in.';
                    header('Location: /login');
                    exit;
                }
                $_SESSION['user'] = [
    ...
    if ($action === 'signup') {
        try {
            ...
            if ($name === '' || !filter_var($email, FILTER_VALIDATE_EMAIL) || strlen($password) < 6) {
                throw new RuntimeException('Please enter valid sign up details.');
            }
            ...
            $_SESSION['flash'] = 'Error creating account.';
            header('Location: /signup');
            exit;
        }
    }
    if ($action === 'contact') {
        try {
            ...
            if ($name === '' || !filter_var($email, FILTER_VALIDATE_EMAIL) || $message === '') {
                throw new RuntimeException('Please fill out all contact fields.');
            }
    ...
    if ($action === 'donate' || $action === 'subscribe') {
        try {
            if (!$user && $action === 'subscribe') {
                throw new RuntimeException('Please log in before subscribing.');
            }
            ...
            $_SESSION['flash'] = 'Payment start failed: ' . $e->getMessage();
        }
        header('Location: /' . ($action === 'donate' ? 'donate' : 'members'));
        exit;
    }
}
...
function public_app_url(string $path = ''): string {
    ...
    if ($base === '') {
        ...
        $base = $scheme . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost') . app_base_path();
    }
    return $base . '/' . ltrim($path, '/');
}

function send_verification_email(array $account, string $token): bool {
    $link = public_app_url('/verify-email?token=' . rawurlencode($token));
    $body = "Hello {$account['name']},\n\nPlease verify your Royal Family TZ email: {$link}\n\nRoyal Family TZ";
    return send_email((string)$account['email'], 'Verify your Royal Family TZ email', $body);
}
...
function ensure_contact_columns(): void {
    static $done = false;
    if ($done) return;
    try {
        db()->exec("ALTER TABLE contact_messages ADD COLUMN status ENUM('new','replied') NOT NULL DEFAULT 'new'");
    } catch (Throwable $e) {}
    $done = true;
}

function ensure_trip_tables(): void {
    static $done = false;
    if ($done) return;
    db()->exec("CREATE TABLE IF NOT EXISTS trips (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, title VARCHAR(180) NOT NULL, slug VARCHAR(180) NOT NULL UNIQUE, description TEXT NOT NULL, destination VARCHAR(180) NOT NULL, trip_date DATE NULL, meeting_point VARCHAR(180) NULL, poster_image VARCHAR(255) NULL, published TINYINT(1) NOT NULL DEFAULT 1, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB");
    try {
        db()->exec('CREATE TABLE IF NOT EXISTS trip_packages (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, trip_id INT UNSIGNED NOT NULL, name VARCHAR(120) NOT NULL, description TEXT NOT NULL, price DECIMAL(12,2) NOT NULL, capacity INT UNSIGNED NULL, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT fk_trip_packages_trip FOREIGN KEY (trip_id) REFERENCES trips(id) ON DELETE CASCADE) ENGINE=InnoDB');
    } catch (Throwable $e) {}
    try {
        db()->exec("CREATE TABLE IF NOT EXISTS trip_bookings (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, user_id INT UNSIGNED NOT NULL, trip_id INT UNSIGNED NOT NULL, package_id INT UNSIGNED NOT NULL, transaction_id INT UNSIGNED NULL, guests INT UNSIGNED NOT NULL DEFAULT 1, status ENUM('pending','paid','cancelled') NOT NULL DEFAULT 'pending', created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT fk_trip_bookings_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE, CONSTRAINT fk_trip_bookings_trip FOREIGN KEY (trip_id) REFERENCES trips(id) ON DELETE CASCADE, CONSTRAINT fk_trip_bookings_package FOREIGN KEY (package_id) REFERENCES trip_packages(id) ON DELETE CASCADE, CONSTRAINT fk_trip_bookings_transaction FOREIGN KEY (transaction_id) REFERENCES transactions(id) ON DELETE SET NULL) ENGINE=InnoDB");
    } catch (Throwable $e) {}
    $done = true;
}

function logo_url(): string {
    return app_base_path() . '/assets/royal-family-logo.jpg';
}

function nav(string $current, ?array $user): void {
    $base = app_base_path();
    $links = ['/' => 'Home', '/about' => 'About', '/members' => 'Members', '/trips' => 'Trips', '/blog' => 'Blog', '/contact' => 'Contact', '/donate' => 'Donate'];
    echo '<header><div class="nav"><a class="brand" href="' . $base . '/"><img class="brand-logo" src="' . e(logo_url()) . '" alt="Royal Family TZ logo"><span>Royal Family <small>Tanzania</small></span></a><input class="menu-checkbox" type="checkbox" id="mobile-menu"><label class="menu-toggle" for="mobile-menu" aria-label="Open menu">☰</label><nav data-mobile-nav>';
    foreach ($links as $href => $label) {
        echo '<a class="' . ($current === $href ? 'active' : '') . '" href="' . e($base . $href) . '">' . $label . '</a>';
    }
    if ($user) {
        echo '<a href="' . e($base . '/dashboard') . '">Dashboard</a><a class="outline" href="' . e($base . '/logout') . '">Log out</a>';
    } else {
        echo '<a href="' . e($base . '/login') . '">Log in</a>';
    }
    echo '</nav></div></header>';
}
...
function layout(string $title, string $content, string $path, ?array $user, ?string $flash = null): void {
    ...
    nav($path, $user);
    if ($flash) {
        echo '<div class="flash">' . e($flash) . '</div>';
    }
