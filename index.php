...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'donate' || $action === 'subscribe') {
        ...
        header('Location: /' . ($action === 'donate' ? 'donate' : 'subscribe')); exit;
    }
}
...
case '/members':
        ...
        break;
case '/donate':
    $content = '<div class="page"><div class="center"><p class="eyebrow">Support the mission</p><h1>Donate to Royal Family TZ</h1><p class="lead">Your gift helps fund community projects, youth talent development, and events.</p></div><div class="membership-grid">';
    $donationOptions = [2000=>"Small gift - TZS 2,000",5000=>"Supporter - TZS 5,000",10000=>"Sustainer - TZS 10,000",50000=>"Sponsor - TZS 50,000",100000=>"Partner - TZS 100,000",500000=>"Major gift - TZS 500,000"];
    foreach($donationOptions as $amt=>$label) {
        $content .= '<article class="member-card"><div><span class="tier-badge">Donate</span><h2>'.e($label).'</h2><p class="muted">Thank you for supporting our programs and events.</p><div class="price-option"><div><strong>TZS '.number_format($amt).'</strong></div>' . ($user ? '<form method="post" style="margin:0"><input type="hidden" name="action" value="donate"><input type="hidden" name="amount" value="'.(int)$amt.'"><button class="btn gold" type="submit">Donate</button></form>' : '<a class="btn gold" href="/login">Log in to donate</a>') . '</div></div></article>';
    }
    $content .= '</div><section class="section"><div class="center"><p class="eyebrow">How to pay</p><h2>Choose a payment method</h2></div><div class="card"><form class="form" method="post"><input type="hidden" name="action" value="donate"><label>Mobile number<input name="phone" placeholder="0712345678" required></label><label>Amount (TZS)<input type="number" name="amount" min="1" required></label><label>Choose payment method</label><div class="payment-grid"><div class="payment-option"><input type="radio" name="method" value="mpesa" id="d-mpesa" checked><label for="d-mpesa"><img src="'.e(app_base_path()).'/assets/payments/mpesa.png" class="payment-logo" alt="M-Pesa"><span>Vodacom M-Pesa</span></label></div><div class="payment-option"><input type="radio" name="method" value="card" id="d-card"><label for="d-card"><span>Credit / Debit Card</span><small class="muted">Powered by ClickPesa</small></label></div></div><button class="btn gold" type="submit">Donate now</button></form></div></section></div>';
    break;
...
case '/subscribe': $content='<div class="page"><p class="eyebrow">Membership</p><h1>Choose your plan.</h1><div class="grid">'; foreach($tiers as $t) $content.='<article class="card"><h3>'.e($t['name']).'</h3><h2>'.e($t['price']).'</h2><p>'.e($t['text']).'</p><form class="form" method="post"><input type="hidden" name="action" value="subscribe"><input type="hidden" name="tier" value="'.e($t['name']).'"><input type="hidden" name="amount" value="'.(int)preg_replace('/[^0-9]/','',$t['price']).'"><label>Mobile number<input name="phone" placeholder="0712345678" required></label><label>Choose payment method</label>
...
</div><button class="btn" type="submit">Pay and join</button></form></article>'; $content.='</div></div>'; break;
case '/admin':
